---
allowed-tools: ["Read"]
description: "tool for gathering single process properties through structured interviews"
---


ユーザーからのクエリをもとに ProcessNode のプロパティを補完したmermaid記法のフローチャートを記述して。
process node全体をsubgraphの網掛け, プロパティのinput, outputをノード, とするflowchartをmermaid記法で書いて. あるプロセスのinputがあるプロセスのoutputになる、の関係性も含めて記述を。
ProcessNode含めたデータ構造は以下の通り。ただしprocess_flowは未定義のものを含めて良いので、ProcessNode一つだけを返すものとする。
